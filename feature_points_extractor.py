...
def get_features(original_face):
    ...
    try:
        ...
        # if not init == 'done':
        #     landmark = int(random.randint(1,len(key_points_loc)))
        #     print(landmark)
        #     init = 'done'

        dists = []
        for i in range(len(x)):
            dists.append(get_distance([x[i],y[i]],[x[landmark],y[landmark]]))
            if dists[i] == 0:
                # dists[i] = 10000000000
                print(i," zeroth")
                # print(i)
            # print(key_points_loc[i],key_points_loc[landmark])
        # dists = dists[1:]
